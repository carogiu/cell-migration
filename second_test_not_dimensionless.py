...
def problem_no_div(W_flow, vi, theta, beta, phi):
    ...
    a_flow = beta * theta_p * dot(velocity, v_test) * dx - pressure * div(v_test) * dx + p_test * div(velocity) * dx
    L_flow = f * p_test * dx
...
class InitialConditions(UserExpression):  # result is a dolfin Expression
    ...
    def eval(self, values, x):
        ...
        if abs(x[0] - .5) < xi / 2:
            # random perturbation
            # values[0] = np.tanh(((x[0] - .5) * 2) / (xi * np.sqrt(2))) + np.random.randn(1) * 0.05 # phi(0)
            # sin perturbation
            values[0] = np.tanh(((x[0] - .5) * 2) / (xi * np.sqrt(2))) + np.sin(x[1] * 30) * 0.1

        else:
            values[0] = np.tanh(((x[0] - .5) * 2) / (xi * np.sqrt(2)))
        values[1] = 0.0  # mu(0)
...
def problem_phase_in_progress(phi_test, mu_test, du, u, phi, mu, phi_0, mu_0, velocity, theta, xi, dt, M, kappa):
    f = potential_in_progress(phi)
    mu_mid = mu_calc(theta, mu, mu_0)

    L0 = phi * phi_test * dx - phi_0 * phi_test * dx + dt * dot(velocity,
                                                                grad(phi_0)) * phi_test * dx +  M * dt * dot(
        grad(mu_mid),
        grad(phi_test)) * dx
    L1 = mu * mu_test * dx - kappa * f * mu_test * dx /(xi**2) - kappa * dot(grad(phi), grad(mu_test)) * dx
    L = L0 + L1
...
def time_evolution_phase(ME, W_flow, vi, n, nx, ny, dt, theta, beta, xi, M, kappa):
    # initiate the functions
    phi_test, mu_test, du, u, phi, mu, u0, phi_0, mu_0 = initiate_phase(ME)
    U_flow = problem_no_div(W_flow, vi, theta, beta, phi)
    ...
    for i in range(1, n):
        a, L, u = problem_phase_in_progress(phi_test, mu_test, du, u, phi, mu, phi_0, mu_0, velocity, theta, xi, dt, M, kappa)
        u0.vector()[:] = u.vector()
        u = solve_phase(a, L, u)
        U_flow = problem_no_div(W_flow, vi, theta, beta, phi)
        velocity, pressure = split(U_flow)
...
phi_tot, vx_tot, vy_tot, p_tot = time_evolution_phase(ME, W_flow, vi, n, nx, ny, dt, theta, beta, xi, M, kappa)
see_all(phi_tot)
